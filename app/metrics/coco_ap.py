...
class COCOAP(MetricBase):
    """
    Coco Avg Precision metric class.
    ...
    def calculate(
        ...
        try:
            full_text = '\n'.join(references)  
            refs = json.loads(full_text)
            
            full_text = '\n'.join(hypothesis)  
            hyps = json.loads(full_text)


            stats = evaluate(refs, hyps)
            return stats[0]*100
...
class COCO:
    def __init__(self, annotation_file_json=None):
        """
        ...
        if not annotation_file_json == None:
            print('loading annotations into memory...')
            tic = time.time()
            dataset = annotation_file_json
...
def evaluate(gt_path, pred_path):
    """Evaluate COCO predictions using pycocotools."""
